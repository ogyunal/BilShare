...
import com.bilshare.bilshare.backend.data.entity.util.EntityUtil;
...
import com.bilshare.bilshare.app.HasLogger;
...
@Route(value = BilShareConstants.PAGE_STOREFRONT, layout = MainView.class)
@RouteAlias(value = BilShareConstants.PAGE_STOREFRONT_EDIT, layout = MainView.class)
@RouteAlias(value = BilShareConstants.PAGE_ROOT, layout = MainView.class)
@PageTitle(BilShareConstants.TITLE_STOREFRONT)
public class StorefrontView extends PolymerTemplate<TemplateModel>
	...
	public StorefrontView(OrderPresenter presenter, OrderEditor orderEditor) {
		...
		grid.addColumn(OrderCard.getTemplate()
				...
				.withEventHandler("cardClick",
						order -> UI.getCurrent().navigate(BilShareConstants.PAGE_STOREFRONT + "/" + order.getId())));
	...
	public void setParameter(BeforeEvent event, @OptionalParameter Long orderId) {
		boolean editView = event.getLocation().getPath().contains(BilShareConstants.PAGE_STOREFRONT_EDIT);
		if (orderId != null) {
	...
	void navigateToMainView() {
		getUI().ifPresent(ui -> ui.navigate(BilShareConstants.PAGE_STOREFRONT));
	}
